...
async function getStreams(
  ...
  try {
    ...
    const addonUrl = `${ADDON_URL}/${stremioType}/${encodeURIComponent(id)}.json`
    let payload: StremioResponse | undefined
    let lastError: unknown
    for (let attempt = 1; attempt <= ADDON_ATTEMPTS; attempt++) {
      try {
        ...
        payload = (await response.json()) as StremioResponse
        break
      } catch (error) {
        lastError = error
      }
    ...
    if (!streams.length)
      console.log(
        `[BollyFlix] Addon returned no playable streams for ${stremioType}/${id}`
      )
