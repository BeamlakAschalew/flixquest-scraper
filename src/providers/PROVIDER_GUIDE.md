...
| ----------------- | ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `aether`          | Broad international multi-worker fallback with regional alternatives     | 🌐 Original/🇬🇧 EN; optional 🇲🇽/🇪🇸 ES, 🇩🇪 DE and 🇫🇷 FR workers                          | **Auto HLS** observed                                                                               | Aggregates independent workers and keeps partial successes. Nebula and Meridian worked for both live tests; Tiki also worked for the tested TV episode. Meridian supplied multilingual subtitles. |
| `goated`          | Broad movie/TV aggregator with independent Orbit and Valenox sources     | 🌐 Original or source-provided multi-audio                                               | **360p, 720p, 1080p**, and **2160p/4K** observed                                                    | Returns adaptive HLS masters and short-lived signed URLs. The provider inspects the master ladder but keeps the master intact so alternate audio renditions remain playable.                       |
| `bingr`           | Broad movie/TV multi-server fallback with language-specific sources       | 🌐 Original plus 🇮🇳 Hindi/Multi sources when available                                  | Explicit **480p, 720p, 1080p** observed                                                            | Queries all nine servers and keeps partial successes. Resolution, language, subtitles, and required playback headers are preserved per source.                                                     |
| `rive`            | Broad movie/TV aggregator with independent direct-stream resolvers        | 🌐 Original, 🇮🇳 Hindi, and 🇻🇳 Vietnamese sources vary by resolver                        | Explicit **360p, 480p, 720p, 1080p** plus adaptive HLS observed                                    | Queries Rive's live resolver catalog and keeps partial successes. Citadel supplied 1080p movies; FlowCast supplied 360p/480p/720p TV streams and multilingual subtitles in the live check.          |
| `vidrift`         | Broad movie/TV coverage through the independent VidRift Earth resolver    | 🌐 Original audio                                                                       | Adaptive **360p, 720p, 1080p** observed; some titles expose a subset                               | Uses only VidRift's distinct Earth backend and deduplicates repeated final CDN origins. 7Movies fallback servers that overlap existing providers are intentionally excluded.                       |
| `vuflix`          | Broad movie/TV multi-source catalog including Sigma, 4K, Upsilon and more | 🌐 Original/🇬🇧 EN; optional 🇮🇳 HI and 🇩🇪 DE tracks vary by backend                         | Explicit **360p, 480p, 720p, 1080p, 2160p/4K** plus adaptive HLS observed                         | Tries 4K first, then 4K2, and returns immediately when either supplies streams. The remaining live catalog is queried only when both preferred backends are empty; individual failures remain isolated. |
| `cinevaro`        | Broad movie/TV fallback through Cinevaro's first-party Vaplayer API      | 🌐 Original audio; title-dependent                                                      | Explicit **360p, 480p, 720p, 1080p** tiers are parsed from adaptive HLS when present                 | Uses direct JSON and manifest requests only. Cinemascope dimensions such as 1920x800 are normalized to their conventional delivery tier. Availability varies by title.                            |
| `fsharetv`        | Dedicated movie server with several storage mirrors                      | 🌐 Original audio; title-dependent                                                      | Explicit **360p, 480p, 720p, 1080p** MP4 tiers when present                                          | Movie-only. Resolves the live watch page and JSON source endpoint without a browser, keeps one mirror per quality, and falls back between the current `.co` and `.cc` domains.                    |
| `vyla`            | Broad movie/TV fallback through Vyla's active source API                  | 🌐 Original audio; backend-dependent                                                    | Detects MP4 resolution and parses adaptive HLS tiers up to **1080p**                                 | Uses Vyla's first-party session and source APIs, then returns the origin media URL. Runtime is direct HTTP only; availability and exact quality vary by title and episode.                        |
...
| `bollyflix`       | Bollywood, South Indian cinema, Hindi-dubbed Hollywood, web series       | 🇮🇳 HI commonly; 🇬🇧 EN; regional or dual audio varies                                   | **480p, 720p, 1080p** observed; occasional 2160p is catalog-dependent                               | Resolves the current catalog, season/episode pages, and quality-specific Google-hosted files. Dead or missing mirrors are omitted.                                                                 |
